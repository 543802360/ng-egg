...
@Component({
  selector: 'app-zt_djs',
  templateUrl: './zt_djs.component.html',
  styleUrls: ['./zt_djs.component.scss']
})
export class TaxDatavZtDjsComponent implements OnInit, AfterViewInit {

  ddqydataurl = "datacenter/ddqydata";//独瞪企业
  defaultApiUrl = 'datacenter/default/djs'; //默认数据接口
  style = decimal;
  ...
  */
  ...
  getDjsData(swjg_dm?: string) {
    ...
    // this.http.get('datacenter/djsqydata/test', { SWJG_DM: DEFAULT_SWJG_DM }).subscribe(respZb => {
    //   console.log("resp djs:", respZb);
  ...
  */
  ...
  initRightBottomChart1(nsrmc, dataList) {
    ...
    let item = dataList.find(i => i.nsrmc == nsrmc);
    const pieData = [];
  ...
  */
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      this.taxDataVSrv.title = '独角兽和瞪羚企业分布图';
    });
  }
}
