...
@Component({
  selector: 'app-zt_fdc',
  templateUrl: './zt_fdc.component.html',
  styleUrls: ['./zt_fdc.component.scss']
})
export class TaxDatavZtFdcComponent implements OnInit, AfterViewInit {
  ...
  */
  ...
  getDjsData() {
    this.http.get("http://140.68.16.96:10080/ytswht/api/fdcqy/fdcqyzb?_allow_anonymous=true", this.getDjsCondition()).subscribe(resp => {
      console.log("Djs_resp", resp);
      this.ztDjsList = resp.data;
      console.log("this.ztDjsList", this.ztDjsList);
      this.ztDjsList_Chosen = resp.data.sort(order('BNDSR'));
      ...
      this.initRightBottomChart(firstNsrmc);
    });
  }
  /* [3]
  ** （1）添加城市边界
