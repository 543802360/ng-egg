...
@Component({
  selector: 'app-zt_gsqy_gy',
  templateUrl: './zt_gsqy_gy.component.html',
  styleUrls: ['./zt_gsqy_gy.component.scss']
})
export class TaxDatavZtGsqyGyComponent implements OnInit, AfterViewInit {


  gsgyurl = 'datacenter/gsgydata';//规上工业
  defaultApiUrl = 'datacenter/default/gsqy'; //高新企业
  ...
  */
  ...
  initRightBottomChart1(nsrmc, dataList) {
    ...
    const pieData = [];
    pieData.push({ name: "增值税", value: item.ZZS });
    pieData.push({ name: "消费税", value: item.XFS });
    pieData.push({ name: "企业所得税", value: item.QYSDS });
    pieData.push({ name: "资源税", value: item.ZYS });
    pieData.push({ name: "城镇土地使用税", value: item.CZTDSYS });
    pieData.push({ name: "城市维护建设税", value: item.CSWHJSS });
    pieData.push({ name: "印花税", value: item.YHS });
    pieData.push({ name: "土地增值税", value: item.TDZZS });
    pieData.push({ name: "房产税", value: item.FCS });
    pieData.push({ name: "车船税", value: item.CCS });
    //pieData.push({ name: "车辆购置税", value: item.CLGZS });
    //pieData.push({ name: "烟叶税", value: item.YYS });
    pieData.push({ name: "耕地占用税", value: item.GDZYS });
    pieData.push({ name: "契税", value: item.QS });
    pieData.push({ name: "环境保护税", value: item.HJBHS });
    pieData.push({ name: "其他税收", value: item.QTSS });
