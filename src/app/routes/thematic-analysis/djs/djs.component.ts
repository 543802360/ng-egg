...
@Component({
  ...
  styleUrls: ['./djs.component.less']
})
export class DjsEnterpriseGroupOrderComponent implements OnInit, AfterViewInit, ReuseComponentInstance {
  ...
  //jtfbUrl = `jtqy/jtfb`;
  // 按税务机关排序URL
  jgtjUrl = 'http://140.68.16.96:10080/ytswht/api/djsdlqy/jgtj?_allow_anonymous=true';
  jtpmUrl = 'http://140.68.16.96:10080/ytswht/api/djsdlqy/djsdlqyzb?_allow_anonymous=true';
  jtpmData: itemInfo[];
  jgtjData: itemInfo[];
  ...
  getData() {
    ...
    this.downCount = 0;
    // 按企业税收收入排序
    const $jtpmStream = this.http.get(this.jtpmUrl, this.getCondition());
    $jtpmStream.subscribe(resp => {
    ...
    $jgtjStream.subscribe(resp => {
      ...
      this.jgtjData.forEach(el => {
        if (el.ZGSWJGJC == "烟台经济开发区税务局") {
          el.COUNT = 16;
        } else { }
        if (el.ZGSWJGJC == "烟台市莱山区税务局") {
          el.COUNT = 6;
        } else { }
        if (el.ZGSWJGJC == "栖霞市税务局") {
          el.COUNT = 4;
        } else { }
        if (el.ZGSWJGJC == "蓬莱市税务局") {
          el.COUNT = 4;
        } else { }
        if (el.ZGSWJGJC == "莱州市税务局") {
          el.COUNT = 3;
        } else { }
        if (el.ZGSWJGJC == "龙口市税务局") {
          el.COUNT = 3;
        } else { }
        if (el.ZGSWJGJC == "烟台福山税务局") {
          el.COUNT = 3;
        } else { }
        if (el.ZGSWJGJC == "烟台市芝罘区税务局") {
          el.COUNT = 2;
        } else { }
        if (el.ZGSWJGJC == "烟台保税港区税务局") {
          el.COUNT = 2;
        } else { }
        if (el.ZGSWJGJC == "莱阳市税务局") {
          el.COUNT = 1;
        } else { }
        if (el.ZGSWJGJC == "烟台牟平税务局") {
          el.COUNT = 1;
        } else { }
        if (el.ZGSWJGJC == "烟台高新区税务局") {
          el.COUNT = 1;
        } else { }
        if (el.ZGSWJGJC == "海阳市税务局") {
          el.COUNT = 1;
        } else { }
